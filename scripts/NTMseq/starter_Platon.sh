...
echo -e "Sample\t#Unique Refseq hits" > $PATH_output/"Platon_Output_RefSeqHits_"$set.tsv
...
echo -e "id\trep\tmob_genes\tconj_genes\t" > $PATH_output/mobGenes.tsv

for file in $PATH_output/*/*.log
do
	Counter_UniqueHits=0
	...
	Hit_rep_temp=""
	Hit_rep=""
	echo -e "contig\tid\tc-start\tc-end\tcoverage\tidentity" > $PATH_output/$SampleName/RefSeq_hits.txt
	echo "Summarizing refseq hits, mob and conj genes for "$SampleName
	while read line
	do
		if [[ "$line" == *"ref plasmids: hit!"* ]]
		then
			Hit_contig=$(echo $line | cut -d ',' -f 2 | cut -d '=' -f 2)
			Hit_id=$(echo $line | cut -d ',' -f 3 | cut -d '=' -f 2)
			Hit_start=$(echo $line | cut -d ',' -f 4 | cut -d '=' -f 2)
			Hit_end=$(echo $line| cut -d ',' -f 5 | cut -d '=' -f 2)
			Coverage=$(echo $line| cut -d ',' -f 6 | cut -d '=' -f 2)
			Identity=$(echo $line| cut -d ',' -f 7 | cut -d '=' -f 2)
			paste --delimiter='\t'  <(echo $Hit_contig) <(echo $Hit_id) <(echo $Hit_start) <(echo $Hit_end) <(echo $Coverage) <(echo $Identity) >> $PATH_output/$SampleName/RefSeq_hits.txt
		elif [[ "$line" == *"mob genes: hit!"* ]]
		then
			Hit_mob_temp=$(echo $line | cut -d ',' -f 3 | cut -d '=' -f 2)
			if [ -z "$Hit_mob" ]; then
				Hit_mob=$Hit_mob_temp
			else
				Hit_mob+=",$Hit_mob_temp"
			fi
		elif [[ "$line" == *"conj genes: hit!"* ]]
			then
			Hit_conj_temp=$(echo $line | cut -d ',' -f 3 | cut -d '=' -f 2)
			if [ -z "$Hit_conj" ]; then
				Hit_conj=$Hit_conj_temp
			else
				Hit_conj+=",$Hit_conj_temp"
			fi
		elif [[ "$line" == *"rep genes: hit!"* ]]
		then
			Hit_rep_temp=$(echo $line | cut -d ',' -f 3 | cut -d '=' -f 2)
			if [ -z "$Hit_rep" ]; then
				Hit_rep=$Hit_rep_temp
			else
				Hit_rep+=",$Hit_rep_temp"
			fi
		fi
	done <<< "$(cat $file)"
	Counter_UniqueHits=$(tail -n +2 $PATH_output/$SampleName/RefSeq_hits.txt | sort | uniq | wc -l) #count unique number of refseq hits. Remove first the header!
	paste --delimiter='\t'  <(echo $SampleName) <(echo $Counter_UniqueHits) >> $PATH_output/"Platon_Output_RefSeqHits_"$set.tsv
	paste --delimiter='\t'  <(echo $SampleName) <(echo $Hit_rep) <(echo $Hit_mob) <(echo $Hit_conj) >> $PATH_output/mobGenes.tsv
done 
