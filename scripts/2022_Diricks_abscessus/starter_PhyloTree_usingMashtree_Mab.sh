...
echo "Don´t forget to change parameters such as path to input/output files, e.g. using notepad++"
...
TempDir="" #Path to directory where temporary files are stored (change if you have space disk issues). Note that all final files will be moved from this folder to your output folder.
...
SampleCount=$(ls -l $PATH_input/*$FileType | wc -l)
